    ...
    public class ELCImagePickerViewController : UINavigationController
    {
        ...
        public class ELCAlbumPickerController : UITableViewController
        {
            ...
            async public override void ViewDidLoad()
            {
                ...
                var locationPermission = await Permissions.RequestAsync<Permissions.Photos>();
                ...
                library.Enumerate(ALAssetsGroupType.Event, GroupsEnumerator, GroupsEnumeratorFailed);
            ...
            public override void ViewDidAppear(bool animated)
            {
                ...
                //        NavigationController.PushViewController(picker, false);
                //    }
                //}
            }
            ...
            void GroupsEnumerator(ALAssetsGroup agroup, ref bool stop)
            {
                ...
                agroup.SetAssetsFilter(ALAssetsFilter.AllAssets);
...
#region UICollectionViewDataSource
            ...
            public override UICollectionViewCell GetCell(UICollectionView collectionView, NSIndexPath indexPath)
            {
                ...
                var viewexist = cell.ViewWithTag(20000);
                if (viewexist==null)
                {
                    var v = new UIView(new CGRect(cell.Frame.Size.Width - 26, 2, 22, 22));
                    var lbl = new UILabel(new CGRect(0, 0, 22, 22));
                    lbl.Tag = 100000;
                    lbl.Font = UIFont.SystemFontOfSize(9);
                    lbl.TextColor = UIColor.White;
                    lbl.Text = selectedImgCount.ToString();
...
#region Not interested in
            ...
            void AssetSelected(NSIndexPath targetIndexPath, bool selected)
            {
                ...
                var cell = CollectionView.CellForItem(targetIndexPath);
                var viewexist = cell.ViewWithTag(20000);
                if (selected)
                {
                    selectedImgCount = selectedImgCount + 1;
                    var lbl = (UILabel)viewexist.ViewWithTag(100000);
                    lbl.Text = selectedImgCount.ToString();
                    viewexist.Hidden = false;
                ...
                else
                {
                    selectedImgCount = selectedImgCount - 1;
                    var lbl = (UILabel)viewexist.ViewWithTag(100000);
                    viewexist.Hidden = true;
                    ResetSelectedCountNumber(Convert.ToInt32(lbl.Text));
                    lbl.Text = 0.ToString();
                }
            }

            void ResetSelectedCountNumber(int selectCount)
            {
                var list = CollectionView.NumberOfItemsInSection(0);
                for (int i=0; i < list; i++)
                {
                    ...
                    if (cell != null)
                    {
                        var viewexist = cell.ViewWithTag(20000);
                        var lbl = (UILabel)viewexist.ViewWithTag(100000);
                        if (Convert.ToUInt16(lbl.Text) > selectCount)
                        {
                            lbl.Text = (Convert.ToUInt16(lbl.Text) - 1).ToString();
                            if (lbl.Text == "0")
                                viewexist.Hidden = true;
                        }
                    }
